...
import { fetchEvents, addEvent, updateEvent, deleteEvent } from '../store/slices/eventsSlice';
...
export const CalendarPage: React.FC = () => {
  ...
  const handleCreateEvent = async (eventData: Omit<Event, 'id'>) => {
    ...
    if (selectedDate) {
      ...
      const timeFromForm = new Date(eventData.date);
      
      finalDate = new Date(
        selectedDateOnly.getFullYear(),
        selectedDateOnly.getMonth(),
        selectedDateOnly.getDate(),
        timeFromForm.getHours(),
        timeFromForm.getMinutes()
      );
    }
    ...
    try {
      await dispatch(addEvent(newEvent)).unwrap();
      setModalType(null);
